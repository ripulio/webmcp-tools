export const tool = {
  ...
  description: 'Get the list of product search results on the current Amazon search page. Returns product titles, prices, ratings, and ASINs.',
  inputSchema: {
    type: 'object',
    properties: {
  ...
  async execute(rawInput: { limit?: number }) {
    ...
    for (const el of resultElements) {
      ...
      const priceEl = findElement(el, ['.a-price .a-offscreen', '.a-price-whole', '.a-color-price']);
      ...
      // Rating extraction
      const ratingEl = findElement(el, ['.a-icon-alt', '[aria-label*="stars"]']);
      const rating = ratingEl?.textContent?.trim() || ratingEl?.getAttribute('aria-label') || null;

      // Review count extraction
      const reviewCountEl = findElement(el, ['a[href*="#customerReviews"] span', '[aria-label*="reviews"]', '.a-size-small .a-link-normal']);
    ...
    const pageSelectors = ['.s-pagination-selected', '.a-pagination .a-selected', '[aria-current="page"]'];
    let currentPage = '1';
    ...
    return {
      content: [{ type: 'text' as const, text: `Found ${results.length} results (Page ${currentPage}):\n\n${summary}` }],
      structuredContent: { results, currentPage: parseInt(currentPage, 10), totalReturned: results.length }
    };
  }
};
