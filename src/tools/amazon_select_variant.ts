export const tool = {
  ...
  description:
    'Select a product variant (color, size, style) on an Amazon product page. Use get_product_details first to see available variants.',
  inputSchema: {
    type: 'object',
    properties: {
      variantType: {
      ...
      value: {
        type: 'string',
        description:
          'The variant value to select (e.g., "Black", "Large", "2-Pack")'
      }
    },
  ...
  async execute(rawInput: {variantType?: string; value?: string}) {
    ...
    if (!variantType || !value) {
      return {
        content: [
          {
            type: 'text' as const,
            text: 'Both variantType and value parameters are required.'
          }
        ],
        isError: true
      };
    ...
    if (!selectors) {
      return {
        content: [
          {
            type: 'text' as const,
            text: `Invalid variant type: ${variantType}. Use "color", "size", or "style".`
          }
        ],
        isError: true
      };
    ...
    if (!variantElements || variantElements.length === 0) {
      return {
        content: [
          {
            type: 'text' as const,
            text: `No ${variantType} variants found on this product page. This product may not have ${variantType} options.`
          }
        ],
        isError: true
      };
    ...
    if (!matchedVariant) {
      return {
        content: [
          {
            type: 'text' as const,
            text: `${variantType} "${value}" not found. Available options: ${availableOptions.slice(0, 10).join(', ')}`
          }
        ],
        isError: true
      };
    ...
    // Click the variant
    const clickTarget =
      matchedVariant.querySelector('button, a, img') || matchedVariant;
    (clickTarget as HTMLElement).click();

    return {
      content: [
        {type: 'text' as const, text: `Selected ${variantType}: ${value}`}
      ],
